/***
Fichero: posada_hab3.c
Autor: kastwey
Descripción: Dormitorio 3 de la posada.
Creación: 05/12/2004
***/
...
create()
{
    ...
    SetIntLong(W("Te encuentras en la habitación 3 de la posada de Siempreunidos.\n"
    ...
    SetOficina(HAB_PUESTO("posada/posada_pasillo2"));
    SetIndoors(1);
    SetIntBright(25);
...
int ir_pasillo()
{
    if (TP->move(HAB_PUESTO("posada/posada_pasillo2"), M_SPECIAL, ({"abre la puerta, sale de la habitación y cierra tras de sí", "sale de la habitación cerrando a su espalda", "Abres la puerta, sales de la habitación y vuelves a cerrar.\n", "cadena4", "cadena5", "cadena6"})) != ME_OK)
	{
...
{
    if (!query_wiz_level(PO) && method == M_TELEPORT || (method == M_SILENT && extra = 0) && !present("llave_posada_faeria_3", PO))
