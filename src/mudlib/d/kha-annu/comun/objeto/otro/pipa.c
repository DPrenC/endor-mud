...
create () {
 ::create();
  ...
  SetShort("una pipa");
  SetLong(W("Es una bonita pipa de madera muy bien tallada para fumar.\n"));
  SetIds(({"pipa"}));
...
int ffumar(string str)
{
	if (!str || str=="tabaco")
	{
		...
		if(tabaco)
		{
			...
			"personal.\n"));
			say(W(CAP(TNAME)+" coge algo de tabaco, lo pone en su pipa, lo "
		...
		{
			notify_fail(W("No te queda tabaco que fumar.\n"));
			return 0;
		}
	}
	else
