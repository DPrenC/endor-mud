...
int ir_bosque_so(){
	...
	TP->move(BOSQUE_NYN("bosque206"),M_SILENT);
...
int salir_bosque_so(){
	 ...
	 say(NOMBRE+" se va hacia el sur.\n");
            TP->move(BOSQUE_NYN("cerrado"),M_SILENT); // Hasta que no se abra el camino
...
create() {
	::create();
	  ...
	  AddExit("sur",SF(salir_bosque_so));
	  AddExit("norte",SF(ir_bosque_so));
}
