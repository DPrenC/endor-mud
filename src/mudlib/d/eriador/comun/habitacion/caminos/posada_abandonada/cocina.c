/*****************************************************************************
 AUTOR:		OROME
 DETALLES:	COCINA DE LA POSADA ABANDONADA
 *****************************************************************************/
...
public int IrAlmacen(){
  if (despensa) TP->move(CAMINOS"posada_abandonada/almacen_cocina",M_GO,"sur");
  else {
	write("Comienzas a caminar hacia la puerta y te percatas de que está cerrada.\n");
	say(CAP(TNAME)+" comienza a caminar hacia el sur y se queda parado frente a la puerta.\n",TP);
  }
  return 1;
}
...
public int PulsarPalanca(string str){
  if (str=="palanca"){
	if (despensa){
		...
		say(CAP(TNAME)+" mueve una palanca y la puerta de la despensa se cierra.\n",TP);
		tell_room(CAMINOS"posada_abandonada/almacen_cocina", "La puerta se cierra.\n");
		despensa= 0;
	}
	else{
		write("mueves la palanca y la puerta de la despensa se abre.\n");
		say(CAP(TNAME)+" mueve una palanca y la puerta de la despensa se abre.\n",TP);
		tell_room(CAMINOS"posada_abandonada/almacen_cocina", "La puerta se abre.\n");
		despensa= 1;
	}
	return 1;
  }
...
public int AbrirCerrarPuerta(string str){
  if (str=="puerta"){
	...
	say(CAP(TNAME)+ " mira atentamente la puerta de la despensa.\n",TP);
	return 1;
  }
  return 0;
}
