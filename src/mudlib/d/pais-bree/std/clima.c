/*
 * Servidor de clima para  el dominio de País de Bree
 */
...
create() {
 ...
 "El viento se llevó las nubes y el cielo esta claro.*\n"           /* 11 */
 }));

 /* Lluvia 1 */
  SetDayMessageForClimate(LLUVIA,({
 "El cielo esta claro. Sopla un débil viento cálido.*\n",           /* 0 */
 "Solo hay unas pocas nubes. El viento es cada vez mas fuerte.*\n", /* 1 */
 "Está nublado.*\n",                                                /* 2 */
 "Va a llover.*\n",                                                 /* 3 */
 "Hay algunas gotas de lluvia en el aire.*\n",                      /* 4 */
 "Está lloviendo.*\n",                                              /* 5 */
 "Llueve a cántaros. Mejor que busques cobijo.*\n",                 /* 6 */
 "Está lloviendo a mares. Te has puesto como una sopa.*\n",         /* 7 */
 "Está empezando a escampar.*\n",                                   /* 8 */
 "Aún está lloviendo.*\n",                                          /* 9 */
 "Sólo caen unas pocas gotas.*\n",                                  /* 10 */
 "Ha parado de llover.*\n"                                          /* 11 */
 }));
