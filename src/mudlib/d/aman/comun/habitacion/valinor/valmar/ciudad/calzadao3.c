...
int ir_oeste() {
    if (IS_MAIA(TP)) {
        TP->move(HAB_VALMAR("ciudad/mahanaxar"), M_SPECIAL,
        ({"entra con paso majestuoso en el anillo del Juicio",
              "entra con paso majestuoso en el anillo del Juicio",
              "Penetras en el Máhanaxar con la solemnidad y majestad propias de "
        "uno de los encumbrados señores de Arda que eres"}));
        return 1;
    } else {
        write("Solo los Señores de Arda y quienes ellos convoquen pueden entrar en el "
        "Anillo del Juicio.\n");
        return 1;
    }
    }
...
{    ::create();
    	...
    	SetIndoors(0);
    	    ...
    	    AddDetail(({"llanuras", "llanura", "ondulantes", "ondulante"}),
    	    	...
    	    	"Hacia el oeste la calzada da acceso a un verde montículo coronado por una "
