...
public varargs mixed ChkEquip(object wielder, object equipment, int flags)
{
	...
	{
		tell_object(wielder, "Sólo aquellos que siguen la senda del Bien son dignos de empuñar esta espada.\n");
		return EQ_FORBID;
	}
	if (-1 < member(({"draconiano", "duende", "elfo-oscuro", "enano", "gnomo", "kender", "orco"}), wielder->QueryRace()))
	{
