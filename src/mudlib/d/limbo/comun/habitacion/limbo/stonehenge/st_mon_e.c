/*
DESCRIPCION  : Este del circulo interior de Stonehenge
FICHERO      : /d/limbo/comun/habitacion/limbo/limbo/st_mon_e.c
...
create() {
  ::create();
  SetIntShort("el Este del círculo interior de Stonehenge.");
  SetIntLong("Te encuentras en el circulo interior de Stonehenge. Frente a ti se abre un area "
"circular que forma el corazon de Stonehenge, y al oeste, en su centro, se alza "
...
AddExit("noroeste",STONE("st_mon_n"));
AddExit("este",STONE("st_agua"));
AddExit("suroeste",STONE("st_mon_s"));
AddExit("oeste",STONE("monolito"));
}
