/*
DESCRIPCION  : Norte del circulo interior de Stonehenge
FICHERO      : /d/limbo/comun/habitacion/limbo/limbo/st_mon_n.c
...
create() {
  ::create();
  SetIntShort("el Norte del círculo interior de Stonehenge.");
  SetIntLong("Te encuentras en el circulo interior de Stonehenge. Frente a ti se abre un area "
"circular que forma el corazon de Stonehenge, y al sur, en su centro, se alza "
...
AddExit("sudeste",STONE("st_mon_e"));
AddExit("norte",STONE("st_aire"));
AddExit("suroeste",STONE("st_mon_o"));
AddExit("sur",STONE("monolito"));
}
