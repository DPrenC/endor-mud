/*
DESCRIPCION  : Sur del circulo interior de Stonehenge
FICHERO      : /d/limbo/comun/habitacion/limbo/limbo/st_mon_s.c
...
create() {
  ::create();
  SetIntShort("el Sur del círculo interior de Stonehenge.");
  SetIntLong("Te encuentras en el circulo interior de Stonehenge. Frente a ti se abre un area "
"circular que forma el corazon de Stonehenge, y al norte, en su centro, se alza "
...
AddExit("nordeste",STONE("st_mon_e"));
AddExit("sur",STONE("st_tierra"));
AddExit("noroeste",STONE("st_mon_o"));
AddExit("norte",STONE("monolito"));
}
