/*
DESCRIPCION : tienda del pueblo de Narad
...
{
    ::create();
   SetIntShort("la tienda del pueblo de Narad");
    SetIntLong("Estás en la tienda que hay en el pueblo de Narad. Su propietario es Faust, "
        "un viejo orco. El local es pequeño pero acogedor, y en sus estanterías podrás "
        "encontrar un poco de todo, ya que Faust se dedica a comerciar con productos de uso "
        "general.\n");
    ...
    AddDetail(({"estanterias","productos","estanterías"}),"Las estanterías ocupan gran parte de "
        ...
        "en sus lajas puedes observar los mas variados productos. Usa "+TC_BOLD+"lista"+
        TC_NORMAL+"para conocer que productos están a la venta.\n");
    SetIntNoise((sk_in_shop()?0:"Oyes a Faust trajinar con trastos en el almacén de la "
        "tienda.\n"));
    ...
    RemoveDustbin();
    SetShopkeeper(PNJ_NARAD("faust"));
    SetLastDest("./calle02");
    SetNotifyExits("norte");
    AddExit("norte",NARAD("calle02"));
    ...
    AddArticle(OTRO("frasco"), REFRESH_REMOVE,7);
