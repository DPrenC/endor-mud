/*
DESCRIPCION  : Cruce en el camino a los Campos de cultivo.
FICHERO      : /d/akallab/comun/habitacion/akallab/zarkam/c_camp08.c
...
protected void updateDescr()
{
 SetIntLong(
   ...
   mirar_trigo()+
   "Te encuentras en un cruce en el camino que atraviesa estos campos. En este punto "
   "del camino parte un sendero hacia el norte de la via principal, que conduce "
   "hasta la cabanya destinada a los esclavos que trabajan en los campos.\n");
}
...
create() {
  ::create();
  SetIntShort("el cruce hacia la cabaña");
  updateDescr();
 ...
 AddDetail(({"tierras de cultivo","cultivos","campos de cultivo"}),
  ...
  AddRoomCmd("mirar","mirar_cmd");
  AddRoomCmd("m","mirar_cmd");
  ...
  SetIndoors(0);
  AddExit("norte",Z_CAMPOS("c_camp07"));
  AddExit("oeste",Z_CAMPOS("c_camp09"));
  AddExit("este",Z_CAMPOS("c_camp06"));
  AddExit("sur",Z_CAMPOS("huerta_02"));
}
