/*
DESCRIPCION : cruce de calles junto a la puerta norte
FICHERO     : /d/akallab/comun/habitacion/akallab/zarkam/ciudad/calle_05.c
...
create() {
 ::create();
 SetIntShort("un cruce de calles junto a la puerta norte");
 SetIntLong(
     "Te hallas en el cruce entre la calle mayor de Zarkam, que va de norte a sur, y "
     "una calle secundaria que avanza junto a la muralla norte de la ciudad. Al norte "
     "de aqui queda la entrada a la ciudad formada por dos altas torres unidas por una "
     "pasarela. Desde aqui puedes apreciar las dos grandes puertas que forman esta "
     "entrada.\n");

 AddDetail(({"calle","calles"}),QueryIntLong());

 AddDetail(({"entrada"}), "La entrada a la ciudad queda al norte de aqui.\n");
 ...
 AddDetail(({"puertas"}),
  ...
  AddExit("norte",Z_CIUDAD("calle_06"));
  AddExit("oeste",Z_CIUDAD("calle_23"));
  AddExit("este",Z_CIUDAD("calle_24"));
  AddExit("sur",Z_CIUDAD("calle_04"));
}
