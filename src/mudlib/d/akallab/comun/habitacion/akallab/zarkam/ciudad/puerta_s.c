/*
DESCRIPCION  : Puerta Sur de Zarkam.
FICHERO      : /d/akallab/comun/habitacion/akallab/zarkam/ciudad/puerta_s.c
MODIFICACION : 24-03-98 [Angor@Simauria] Creacion.
...
if ((!present("rompehuesos", TO))&& (!present("masoch", TO)))
   return TP->move(Z_CIUDAD("calle_00"),M_SPECIAL,
   ({"se va hacia el norte","llega desde el sur",
   ...
   if (TP->QueryRace()=="orco"){
     ...
     //Si no eres orco necesitas un salvoconducto para entrar por la noche
        ...
        if (present("salvoconducto_zarkam", TP)){
           say(
              TNAME + " se acerca a la puerta de la ciudad de Zarkam, pero cuando se dispone "
              "a cruzarla es detenid"+AO+" por el guardia. Este da un par de vueltas a su "
              "alrededor, examinandol"+AO+" asi como a su equipo. Tras una breve conversación " +
...
create() {
  ::create();
  SetIntShort("la puerta sur de Zarkam");
  SetIntLong(
     "Estas frente a la puerta sur de la ciudad fortificada de Zarkam. La alta muralla "
     "de piedra que rodea Zarkam tiene aqui una inmensa puerta de dos hojas de metal "
     "macizo. Junto a la puerta suele haber una guardia formada por dos corpulentos "
     "soldados orcos armados hasta los dientes que detiene e interroga a cualquiera "
     "con aspecto sospechoso que intente entrar en la ciudad. El Sendero de los "
     "Cráneos continua a partir de aqui hacia el sur.\n");
...
// AddItem(PNJ("rompehuesos"), REFRESH_DESTRUCT);
  AddExit("norte",SF(entrar));
  AddExit("sur",Z_CAMINO("cc_04"));
