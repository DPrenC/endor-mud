...
FICHERO      : /d/akallab/comun/pnj/std/troll.c
...
create() {
 ...
 if (random(100)<TROLL_AGRESIVO) SetAggressive(1);
 if (random(100)<TROLL_MERODEADOR) SetGoChance(random(10)+25);
 else {
    SetGoChance(0);
    if (random(3)==0){
       InitChats(10,({
          troll+" te mira desconfiad"+AO2+".\n",
          troll+" te dice:'Eh tu, esta es mi cueva, largate de aqui.'\n",
          troll+" arregla un poco la hierba del camastro.\n",
          troll+" saca algo de comida de alguna parte y se pone a comersela.\n",
          troll+" te dice:'No has visto las marcas? esta cueva es mia.'\n",
