...
private int aLState, aInvis, aActions, aMagicDef, aIsPet, aBando;
private int aHidden, aAlign;
private string aName, aRace, aCitizenship, aRazabase,aBandoTxt;
...
public string SetRazabase(string s) { return aRazabase = s; }
public string QueryRazabase() { return aRazabase; }
...
public varargs string *QueryMsgIn (int sc) {
  ...
  switch (QueryRazabase()) {
case "humano": return aMsgIn || ({ "llega", "Alguien llega" });
case "enano": return aMsgIn || ({ "llega caminando pesadamente", "Escuchas unos pesados pasos que se acercan" });
case "elfo": return aMsgIn || ({ "llega con movimientos elegantes", "Oyes unos pasos que se acercan" });
case "hobbit": return aMsgIn || ({ "llega con paso liviano", "oyes unos pasos sigilosos que se acercan" });
	case "alto-hombre": return aMsgIn || ({ "llega a grandes trancos", "Escuchas unos pasos que se acercan" });

// resto de razas.
default:     return aMsgIn || ({ "llega", "Escuchas unos pasos que se acercan" });
}

  switch (sc) {
    case LSTATE_FROG: return ({ "llega saltando", "Oyes como unos saltitos" });
    case LSTATE_GHOST: return ({ "aparece en forma de espectral figura", "Sientes un soplo gélido que te hiela el alma" });
  }
  return ({ "llega", "Oyes unos pasos que se acercan" });
}
...
public varargs string *QueryMsgOut (int sc) {
  if (sc || (sc = QueryLState()) == LSTATE_NORMAL)
switch(QueryRazabase()){
case "alto-hombre":     return aMsgOut || ({ "se va a grandes trancos", "Oyes lentos pasos que se alejan" });
case "enano":     return aMsgOut || ({ "se va caminando pesadamente", "Oyes unos pesados pasos que se alejan" });
case "elfo":     return aMsgOut || ({ "se va con movimientos elegantes", "Oyes unos leves pasos que se alejan" });
case "hobbit":     return aMsgOut || ({ "se va con paso liviano", "Escuchas unas ligeras pisadas que se alejan" });
// resto de razas.
default:    return aMsgOut || ({ "se va", "Oyes unos pasos que se alejan" });
}

  switch (sc) {
    case LSTATE_FROG: return ({ "se va saltando", "Oyes como unos saltitos" });
    case LSTATE_GHOST: return ({ "desaparece cuando su espectral figura es disipada por un soplo intangible", "Sientes como la frialdad que inundaba tu corazón desaparece"});
  }
  return ({ "se va", "Oyes unos pasos que se alejan" });
}
...
public varargs string *QueryMMsgIn (int sc) {
  if (sc || (sc = QueryLState()) == LSTATE_NORMAL)
    return aMMsgIn || ({ "se iergue ante ti revestido de todo su poder"
                       , "Sientes que un gran poder acaba de manifestarse a tu lado" });
  switch (sc) {
    case LSTATE_FROG: return ({ "cae del cielo golpeando el suelo, y maldice a las cigüeñas"
  ...
  return ({ "se iergue ante ti revestido de todo su poder", "Sientes que un gran poder acaba de hacer acto de presencia junto a ti" });
}
...
public varargs string *QueryMMsgOut (int sc) {
  if (sc || (sc = QueryLState()) == LSTATE_NORMAL)
    return aMMsgOut || ({ "alza una mano y pronuncia una palabra de poder, desapareciendo con un relámpago fulgurante"
                        , "Sientes que el gran poder presente ha desaparecido" });
  switch (sc) {
    case LSTATE_FROG: return ({ "es capturado de repente por una cigüeña"
                              , "Algo rápido y alado pasa rozándote" });
    case LSTATE_GHOST: return ({ "se disipa en el viento"
                               , "Parece que el viento suspira tristemente. La gelidez que invadía tu corazón desaparece" });
  }
  return ({ "alza la mano pronunciando una palabra de poder y desaparece con un relámpago blanquecino", "Sientes que el gran poder presente en este lugar desaparece" });
}
...

{
  ...
  if (i = sizeof(strs = _armours()))
  {
    ...
    result[3] = "Está ataviad"+QueryGenderEndString()+" con "+str+".\n";
  }
  ...
  if (i = sizeof(strs = _others()))
  {
    ...
    result[6] = "Está ataviad"+QueryGenderEndString()+" con "+str+".\n";
  }
...
public int QueryBando()          { return aBando; }
public int SetBando(int i)       { return aBando = i; }
public string QueryBandoTxt ()        {
    switch(QueryBando()){
        case P_BANDO_LUZ: aBandoTxt= P_BANDO_COLOR_LUZ+P_BANDO_LUZ_TXT+TC_NORMAL;break;
        case P_BANDO_OSCURIDAD: aBandoTxt= P_BANDO_COLOR_OSCURIDAD+P_BANDO_OSCURIDAD_TXT+TC_NORMAL; break;
        case P_BANDO_NEUTRO:
        default: aBandoTxt= P_BANDO_COLOR_NEUTRO+P_BANDO_NEUTRO_TXT+TC_NORMAL; break;
        }
    return aBandoTxt; }
