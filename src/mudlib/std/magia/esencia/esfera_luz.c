...
#define ESFERA   "/guilds/conjuradores/obj/esfera"

create() {
  ::create();
  SetSpellName("Esfera Luminosa");
  SetSpellUse("");
  ...
  SetSpellXP(9);
  SetSpellDescription(
"Creas una esfera luminosa que te sigue flotando.\n");
 }
...
int Cast(object caster, mixed mix) {
 object esfera;
  ...
  tell_room(environment(caster),W("Aparece una esfera luminosa que se posa encima de tí.\n"));
...
int Fail(object caster, mixed mix) {
  ...
  write("Algo ha debido fallar porque tu no ves que haya más luz.\n");
