 ...
 *
 *     AddDetail("portal", SF(PortalDetail));
 *
...
/* Our variables */
private static string pRoom;
private static mixed pRace;
private static string * pPDetail, *pPSingle, *pPMulti;
private static object counterroom;
private static closure pMayPass;
...
protected void logit (string * others)
{
  ...
  log_file(TP->QueryIsPlayer() ? "portal" : "portal.npc"
...
protected int CheckRace(object player)
{
  ...
  r = QueryPortalRace();
  if (stringp(r))
    return player->QueryRace() == r;
  return -1 != member(r, player->QueryRace());
}
...
// Let TP exit through the mirror (assume it is allowed to do
...

{
  ...
  rc = TP->move(there, M_SPECIAL
               , ({ pPSingle[2], GetOtherRoom()->QueryPortalSingle()[3]
                  , pPSingle[4] })
               );
  if (rc != ME_OK)
  {
    ...
    msg = object_name(TO)+" failed to move "+object_name(TP)+" to "
...

{
  ...
  tmp = filter(found, SF(stringp));
  found -= tmp;
  // found, tmp ->
  //   tmp := tmp + Shorts of all non-livings
  //   founds := all livings
  tmp2 = filter(found, SF(living));
  ...
  {
    rc = found[i]->move(there, M_SILENT);
    if (ME_OK != rc)
    {
      msg = object_name(TO)+" failed to move "+object_name(found[i])+" to "
            +QueryPortalRoom()+", code "+rc+"\n";
      catch(load_object(CHANNELMASTER)->SendMsg("error", msg));
      tmp += ({ found[i] });
      found[i] = 0;
  ...
  ChangeSP(TP, j);
  logit(map(found, SF(object_name)));
  ...
  case 0:
    if (!TP->Query(P_INVIS))
      tell_room(environment(TP), MakeMsg(pPMulti[2]), ({ TP }) +found);
    break;
  case 1: case 2: case 3: case 4:
    tell_room(environment(TP), MakeMsg(pPMulti[3], tmp2), ({ TP }) +found);
    break;
  default:
    tell_room(environment(TP), MakeMsg(pPMulti[4]), ({ TP }) +found);
    break;
  }
  ...
  case 0:
    if (!TP->Query(P_INVIS))
      tell_room(environment(TP), MakeMsg(GetOtherRoom()->QueryPortalMulti()[5]), ({ TP })+found);
    break;
  case 1: case 2: case 3: case 4:
    tell_room(environment(TP), MakeMsg(GetOtherRoom()->QueryPortalMulti()[6]), ({ TP })+found);
    break;
  default:
    tell_room(environment(TP), MakeMsg(GetOtherRoom()->QueryPortalMulti()[7]), ({ TP })+found);
    break;
  }
  ...
  default:
    ...
    map(found[i], SF(tell_object), MakeMsg(pPMulti[12]));
    break;
  }
...
// The function returns 0 if TP is of the wrong race.

{
  ...
  tmp = map(explode(arg, " ") - ({ "", 0 }), SF(lower_case));
...

{
  ...
  if (!args)
    return lambda(({SYM(a)}), ({SF(PortalExitFun), SYM(a), 0 }));
  ...
  tmp = map(tmp, SF(lower_case));

  return lambda(({SYM(a)}), ({SF(PortalExitFun), SYM(a), quote(tmp) }));
}
