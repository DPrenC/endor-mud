/* SIMAURIA '/guilds/kisalas/bin/enseñar.c'
   ========================================
   [w] Woo@simauria

   12-12-99 [w] Para que puedan aprender conjuros de otros del gremio.
*/
...
int main( string str )
{
 ...
 seteuid(getuid(TO));
 if (!str)
   return notify_fail(" Uso: enseñar '<nombre_hechizo>' a <jugador>\n"),0;

 if (sscanf(str,"'%s' a %s",conj,jug)!=2)
    return notify_fail(" Uso: enseñar 'nombre_hechizo' a <jugador>\n"),0;

 if (lower_case(jug) == lower_case(TP->QueryRealName()))
   return notify_fail("¿Te intentas enseñar un hechizo?\n"),0;
 ...
 if (!fichero=SPELL(conj))
   return notify_fail("No conoces ese hechizo.\n"),0;
