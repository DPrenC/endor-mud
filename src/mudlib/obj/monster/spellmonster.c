/**
 * SIMAURIA '/std/monster/spellmonster.c'
 */
...
public varargs string MyName(string cap) {
  string res;
  res = QueryName()||QueryShort()||"alguien";
...
public varargs string MyPronoun(string cap) {
  return cap?capitalize(QueryPronoun()||(QueryGender()==2)?"ella":"el"):QueryPronoun();
}
...
public varargs string MyObjective(string cap) {
  return cap?capitalize(QueryObjective()||(QueryGender()==2)?"ella":"el"):QueryObjective();
} // [h] que coño es esto en español?

/* [h] En español no hay genitivos
...
public varargs string VicPronoun(string cap) {
  ...
  if (sizeof(vics = QueryVictims())>1) res = "ellos";
  else res = vics[<1]->QueryPronoun()||((vics[<1]->QueryGender()==2)?"ella":"el");
  return cap?capitalize(res):res;
}
...
public varargs string VicPossessive(string cap) {
  ...
  if (sizeof(vics = QueryVictims())>1) res = "sus";
  else res = vics[<1]->QueryPossessive()||"su";
  return cap?capitalize(res):res;
}
...
public varargs string VicObjective(string cap) {
  ...
  if (sizeof(vics = QueryVictims())>1) res = "ellos";
  else res = vics[<1]->QueryObjective()||((vics[<1]->QueryGender()==2)?"ella":"el");
  return cap?capitalize(res):res;
} //[h] mmm debo repasar mi ingles, que coño es esto
...
public mapping SetSpells(mapping m) {
  ...
  if (mappingp(m)) {
      ...
      chances = ([]);
      while(i--) chances[idx[i]] = (m[idx[i]][S_CHANCE] = m[idx[i]][S_CHANCE]||1);
  }
  return Pspells = m;
}
				...
				int mindam,int maxdam)
{
  ...
  if (!props[S_VICMSG]&&props[S_MAXDAM])
    props[S_VICMSG] =
      ([
	MSG_NORMAL:({"@@MyName|1@@ lanza "+id+" contra ti.\n",
	           "Alguien lanza "+id+" contra ti.\n"}),
	MSG_DEAD:""
       ]);
  spells = QuerySpells();
  spells[id] = props;
...
public int QueryCastChance()	{ return Pcastchance||20;}

public int SetCastChance(int i)	{ return Pcastchance = i;}
...
protected status check_cast(mixed id) {
  ...
  if (!member(delayed,id)) {
      ...
      if (closurep(sp[S_CHECKFUN])) return funcall(sp[S_CHECKFUN]);
      return 1;
  }
  delayed[id]--;
				  ...
				  mixed id)
{
  ...
  closure repulse;
//Nyh modifica la siguiente linea poniendo el present...
  en = filter((QueryEnemies()||({}))+({}),SF(present));
  vic = ({});
  if (nr_victims<0) {
...
public status CastSpell(mixed id) {
  ...
  if (member(sp,S_FUN)) appl = apply(sp[S_FUN],sp,&vic);
  if (!appl) return 0;
  ...
  while(i--) {
    ...
    if (!vic[i]||vic[i]->QueryHP()<0)	{
      if (vic[i]&&premsg1) tell_object(vic[i], ({MSG_SEE, premsg1}));
      if (premsg2) tell_object(env,({MSG_SEE, premsg2}));
    }
  }
  SetVictims(0);
...
public void Attack() {
  ...
  check = random(100)+1;
  if ( !QueryCombatDelay() && !QueryStunned() && QueryEnabled() && Pcastchance>=check )
...
create() {
  ::create();
 SetCastChance(20);
  REGISTER->Register(TO,"SPELLMONSTER");
  return 0;
}
