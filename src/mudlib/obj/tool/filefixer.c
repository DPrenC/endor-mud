...
string *get_filenames(string str)
{
  ...
  if (geteuid(TI) != geteuid()) return ({});
  path = TP->get_path(str);
  basepath = "/" + implode((explode(path,"/")-({""}))[0..<2]+({""}),"/");
  files = get_dir(path)-({".",".."});
  return files?map(files,lambda(({SYM(arg1),SYM(arg2)}),({SF(+),SYM(arg2),SYM(arg1)})),
                         basepath):0;
}
...
fix(string str)
{
  ...
  tmp = map(explode(str," ")-({""}),SF(get_filenames))-({0});
  files = ({});
  for(i = sizeof(tmp);i--;) files += tmp[i];
  if(sizeof(filter(files,SF(xfix))-({1}))) {
