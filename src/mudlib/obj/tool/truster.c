...
QueryShort() {
  if (!PL || invis || !query_wiz_level(PL)) return 0;
  return thing::QueryShort();
}

id(string str) {
  if (!PL || !query_wiz_level(PL)) return 0;
  return thing::id(str);
}
...
init() {
  ...
  if (!PL || !query_wiz_level(PL) || PL != environment(ME)) return;
...
"  your inventory and in your environment.\n"
  ...
  if (!ob) {
    src = (string) MASTER->_get_path(src,PL->QueryRealName());
  ...
  dest = (string) MASTER->_get_path(dest,PL->QueryRealName());
