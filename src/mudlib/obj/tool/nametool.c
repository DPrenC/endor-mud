...
/* Removed by Pumuckel and mad a new QueryShort function.
    ...
    nickfile="/players/"+TP->QueryRealName()+"/.nickfile";
    removeflag=0;
...
init()
{
    if(TP && TP->QueryIsPlayer() && IS_LEARNER(TP))
    {
...
{
    if(TP && TP->QueryName())
        return TP->QueryName()+"'s nametool.";
...
private int update_nicknames()
{
    ...
    nicknames = ({TP->QueryRealName()})+nicknames;
    TP->SetIds(nicknames);
...
int do_nicklist()
{
    ...
    header=TP->QueryName()+"'s nicknames:\n";
    write(header);
...
int do_nickremove()
{
    ...
    if(removeflag)
    {
        write("OK. Removing now the tool and setting your name to "
              +TP->QueryName()+" only again.\n");

        TP->SetIds(({TP->QueryRealName()}));
        if(!remove())
            destruct(TO);
    }
    else
