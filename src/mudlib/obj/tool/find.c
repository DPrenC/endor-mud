...
finddir(dir, pref) {
  ...
  if (newertime) matches = filter(matches, SF(is_newer)/*'*/, dir);
  /* write(sizeof(matches)+" matches after newer\n"); */
  if (type) matches = filter(matches, "is_type", TO, dir);
  /* write(sizeof(matches)+" matches after newer\n"); */
  if (matches && sizeof(matches)) {
	...
	/* write("Doing `"+d+"'\n");*/
	if (catchit) {
	  if (err = catch(TP->command_me(d)))
	    write("* catched "+err+" for "+d+"\n");
	} else TP->command_me(d);
      }
    }
  }
  /* write(sizeof(entries)+" entries to recurse\n");*/
  filter(entries, SF(finddir), dir+"/");
}
