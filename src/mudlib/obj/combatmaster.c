...
//
// retocado ligeramente para ENDOR-MUD por Orome en 2014.
...
public varargs int CalcDamage(object atk, mixed weapon, object def, int chance) {
  ...
  { // Si atacamos con un arma, x es un objeto (el arma)
    ...
    { // es un arma de verdad
        ...
        {
            case      1 ..    499: dam = d2();  break;
            case      500 ..    1499: dam = d3();  break;
            case   1500 ..   2999: dam = d4();  break;
    ...
    default:
        ...
        switch (dam_type)
        {
            ...
            case TD_PENETRACION:   verb = "alancea";      break;
                case TD_LACERACION:   verb = "lacera";      break;
...
public int GolpeDeGracia(object atk, object def, mixed weapon) {
  ...
  verb[0] = TC_COMBAT_ATACO + "Asestas un último ataque mortal a "+capitalize(def->QueryName())+""TC_NORMAL;
  verb[1] = TC_COMBAT_DEFIENDO + capitalize(atk->QueryName())+" te asesta un último ataque mortal"TC_NORMAL;
  verb[2] = TC_COMBAT_OTRO     + capitalize(atk->QueryName())+" asesta un último ataque mortal a "+capitalize(def->QueryName())+""TC_NORMAL;
...
#ifdef DEBUG_NOPEGO
  if (find_object("orome")) dtell("orome",sprintf("\tCM->Attack(%O, %O, %i, %i, %i)\n",weapon,def,bonus_hit,bonus_dam,flags));
#endif
...
#ifdef DEBUG_ELE
  if (intp(weapon) && objectp(atk->QueryHands()[weapon][HAND_WEAPON]) && find_object("orome") )
    dtell("orome",sprintf("[%O] %O->Attack(%O, %O, %O, %O, %O)\ncaller_stack = %O\n",TP,atk,weapon, def, bonus_hit, bonus_dam, flags,caller_stack()));
#endif
...
//  if (find_object("nyh")) dtell("nyh",sprintf("%O->CM->Attack(%O,%O,%i,%i,%i)\n",atk,weapon,def,bonus_hit,bonus_dam,flags));
  ...
  if (res<1) orgdam = dam = 0;   //si no pega
    ...
    atk->AddDanyo(dam);
#endif
    if (objectp(weapon)) weapon->DegradeWeapon((["TOTAL":1+random(dam/2)]), ([]), 100, 0);
  }
  ...
  {
    tell_object(def,"Eres arrancad"+def->QueryGenderEndString()+" bruscamente de tu plácido sueño cuando "
    +atk->QueryShort()+" te ataca.\n");
    tell_object(atk,capitalize(def->QueryName())+" es arrancad"+def->QueryGenderEndString()+
    " bruscamente de su plácido sueño cuando le atacas.\n");
    tell_room(environment(def),capitalize(def->QueryName())+" es arrancad"+
                               def->QueryGenderEndString()+" bruscamente de su plácido sueño cuando "+
                               atk->QueryShort()+" le ataca.\n", ({def,atk}) );

    def->SetStatusNormal();
  }
  else if (def->QueryStatusResting())
  {
    tell_object(def,"Te levantas de un salto cuando "+atk->QueryShort()+" te ataca.\n");
    tell_room(environment(def),def->QueryName()+" se levanta de un salto.\n", ({def}));
    def->SetStatusNormal();
  }
