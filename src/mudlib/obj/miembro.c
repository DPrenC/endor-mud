...
int    SetGender(int i){genero=i; Actualiza();} //por compatibilidad
...
create() {
  string *myname;
  ::create();
  SetShort("un miembro de algún ser");
...
void heart_beat()
{
 ...
 if(contador>=ritmo_putrefaccion)
 {
  if (vida==0) {
      ...
      if ((n<=25)&&(umbral>25)){
          say(capitalize(s)+" esta podrid"+(QueryGenero()==2?"a":"o")+" casi por completo.\n");
          umbral=25;
          }
      else if ((n<=50)&&(umbral>50)){
          say(capitalize(s)+" esta medio podrid"+(QueryGenero()==2?"a":"o")+".\n");
          umbral=50;
          }
