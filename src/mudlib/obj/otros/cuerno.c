/***************************************************************************************
 ...
 AUTOR:	Orome
 FECHA:		05/04/2013
 Descripción:un cuerno básico
 última modificacion 09/04/2013
 */
...
create () {
 ::create();
  ...
  SetShort("un cuerno");
  SetLong("Es el cuerno de algún animal que ha sido tallado y ahuecado para que pueda sonar.\n");
...
public int ftocar( string arg )
{
   ...
   // objeto necesario.
      seteuid( getuid() );
if(!arg || arg!="cuerno")
        return notify_fail("¿Lo qué exactamente?\n", NOTIFY_NOT_VALID);
   ...
   //si no lo lleva empuñado no puede hacerlo sonar
      if(!TO->QueryEquipped())
   {
      write("Intentas soplar el cuerno, pero sin tenerlo empuñado no es sencillo.\n");
      if(!(TP->QueryInvis() || TP->QueryInwiz())){
   ...
   else
   {
	...
	"El cuerno vibra en tus manos cargado de poder y su poderosa llamada se hace oír "
