...
** Update: Nemesis@Simauria Oct'98/Mar'99 Traducción/Castellanización.
*/
...
private static mapping Pobjects;
// ([<player>:([<short>:<nr>])])
// Only objects will be stored from a certain value on
private static string Pguild;
...
public string GetList()
{
  ...
  res = "Lista de Objetos\n"
        "================\n\n"+
        cadjust("Nombre del Jugador",COL_NAME)+" | "+
        cadjust("Objeto",COL_OBJECT)+" | "+
	cadjust("Acumulador",COL_NR)+"\n"+
        mkstr(COL_NAME,"-")+"-+-"+
        mkstr(COL_OBJECT,"-")+"-+-"+
        mkstr(COL_NR,"-")+"\n";
  while(i--)
...
public int allow_enter(int method,mixed extra)
{
  object po;
  po = PO;
  if (query_once_interactive(po)&&!IS_LEARNER(po))
    if (po->QueryGuild()!=QueryGuild())
      return write("Sólo pueden entrar los miembros del gremio.\n"),
                         ME_NO_ENTER;
    else if (po->QueryGuildLevel()<QueryLevel())
      return write("No tienes suficiente nivel para entrar aquí.\n"),
                        ME_NO_ENTER;
...
public void create()
{
  ...
  SetIntLong("Estás en un almacén.\n");
  AddRoomCmd("lista",SF(cmd_list));
}
